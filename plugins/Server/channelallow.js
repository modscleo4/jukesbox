...
export default new Command({
    ...
    usage: 'channelallow [command1] [command2]',
    ...
    async fn(message, args) {
        ...
        if (args.length === 0) {
            return await message.channel.send(i18n('server.channelallow.noArgs', sc?.lang));
        }
        ...
        sc.allowCommands(message, args);
        serverConfig.set(message.guild.id, sc);
        await sc.save(database_url);

        return await message.channel.send(i18n('server.channelallow.success', sc?.lang, {n: args.length}));
    },
});
