...
import { createAudioPlayer, demuxProbe, VoiceConnectionStatus, entersState, createAudioResource, AudioPlayerStatus, NoSubscriberBehavior, AudioResource } from "@discordjs/voice";
...
async function probeAndCreateResource(readableStream: Readable): Promise<AudioResource> {
    const { stream, type } = await demuxProbe(readableStream);
    return createAudioResource(stream, { inputType: type });
}

async function playSong({ client, guild, channel, author, member, sendMessage }: CommandContext, tries: number = MAX_TRIES): Promise<any> {
    ...
    const errorHandler = async (e: Error) => {
        ...
        if (tries > 0) {
            ...
            if (serverQueue.resource!.playbackDuration && serverQueue.song) {
                ...
                if ((serverQueue.resource?.playbackDuration ?? 0) > 10000) {
                    tries = MAX_TRIES + 1;
                }

                serverQueue.song.seek += Math.floor((serverQueue.resource?.playbackDuration ?? 0) / 1000);
                serverQueue.lastPlaybackTime = (serverQueue.resource?.playbackDuration ?? 0) + serverQueue.startTime * 1000;
...
async function findOnYT(song: Song): Promise<Song | null> {
...
class Play extends Command {
    ...
    async fn({ client, guild, channel, author, member, sendMessage }: CommandContext, args: any[]): Promise<CommandReturn> {
        ...
        if (url.match(/youtube.com|youtu.be/gmu)) {
            ...
            const songInfo = await soundcloud.tracks.getV2(url).catch(e => {
                console.error(e);
                return null;
            });
            ...
            const song = new Song({
                ...
                from: 'sc',
                addedBy: author,
                fn: async (url: string) => new Readable().wrap(await soundcloud.util.streamTrack(songInfo)),
            });
