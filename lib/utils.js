 ...
 * @return {Promise<*>}
 */
export async function pageEmbed({client, author}, {title, description = ' ', content, deleteAfter = false}) {
    ...
    return {
        ...
        onReact: async ({reaction, message}) => {
            ...
            await message.edit(buildEmbed(page));
        },
        deleteAfter,
    };
}
...
export async function getGeniusLyrics(apiToken, q) {
    ...
    const root = htmlparser2.parseDocument(lyrics);

    const el = cssSelect.selectOne('div.lyrics', root, {});
