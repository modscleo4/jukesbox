 ...
 * @return {Promise<Map<string, {prefix: string, volume: number}>>}
 */
 ...
 * @param {Object} sc
 * @param {string} sc.prefix
 * @param {number} sc.volume
 * @return {Promise<*>}
 */
export async function saveServerConfig(database_url, guild_id, {prefix, volume}) {
    const db = new DB(database_url);

    if ((await db.query('SELECT id FROM server_configs WHERE guild = $1', [guild_id])).rows.length === 0) {
        await db.query('INSERT INTO server_configs (guild, prefix, volume) VALUES ($1, $2, $3)', [guild_id, prefix, volume]);
    } else {
        await db.query('UPDATE server_configs SET prefix = $2, volume = $3 WHERE guild = $1', [guild_id, prefix, volume]);
    }
}
