...
export function serverConfigConstruct(prefix) {
    return {
        prefix,
        volume: 100,
    };
...
export function queueConstruct(message, volume, songs) {
    return {
...
export async function saveServerConfig(database_url, guild_id, sc) {
    const db = new DB(database_url);
...
export async function pageEmbed(message, {title, description = ' '}, content, deleteAfter = false) {
    ...
    function buildEmbed(page) {
        return new MessageEmbed()
            .setTitle(title)
            .setDescription(`${description}\n\n${content.slice((page - 1) * maxPerPage, page * maxPerPage).map(({name, value}) => `**${name}** - ${value}`).join('\n\n')}`)
 ...
 * @param {String} type
 * @param {String|Boolean} videoEmbeddable
 * @return {Promise<GaxiosResponse<youtube_v3.Schema$SearchListResponse>>}
 */
export async function searchVideo(q, {key, part = ['id', 'snippet',], maxResults = 10, order = 'relevance', regionCode = 'br', type = 'video', videoEmbeddable = true}) {
 ...
 */
export async function videoInfo(id, {key, part = ['id', 'snippet', 'contentDetails'], maxResults = 50, regionCode = 'br'}) {
    if (typeof id === "string") {
        id = [id];
    }
