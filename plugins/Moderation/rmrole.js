...
export default new Command({
    ...
    async fn({client, guild, channel, author, member}, args) {
        ...
        if (!args[0]?.match(/\d + /gm)) {
            return i18n('mod.rmrole.missingUser', sc?.lang);
        }

        if (args.length < 2) {
            return i18n('mod.rmrole.missingRole', sc?.lang);
        }
        ...
        if (!guildMember) {
            return i18n('mod.rmrole.invalidUser', sc?.lang);
        }

        if (roles.includes(undefined)) {
            return await channel.send(i18n('mod.rmrole.invalidRole', sc?.lang, {role: args[roles.indexOf(undefined)]}));
        }

        if (roles.find(r => !r.editable)) {
            return channel.send(i18n('mod.rmrole.nonEditableRole', sc?.lang, {role: roles.find(r => !r.editable).name}));
        }

        await guildMember.roles.remove(roles);
        i18n('mod.rmrole.success', sc?.lang, {user: guildMember.user.id});
    },
});
