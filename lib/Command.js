...
import {adminID, database_url, ignorePermissionsForAdmin} from "../config.js";
 ...
 * @param {{client: import('./Client.js').default, guild: import('discord.js').Guild, channel: import('discord.js').TextChannel, author: import('discord.js').User, member: import('discord.js').GuildMember, sendMessage: Function}} message
 * @param {string[]} args
...
export default class Command {
    ...
    async checkPermissions({guild, channel, author, member}) {
        ...
        if (this.userPermissions) {
            ...
            if (ignorePermissionsForAdmin && author.id === adminID) {
                return true;
            }
    ...
    static async logUsage(command) {
        ...
        await db.query('UPDATE command_stats SET used = used + 1 WHERE command = ?', [command]);
        await db.query('INSERT INTO command_stats (used) VALUES (1) WHERE NOT EXISTS (SELECT 1 FROM command_stats WHERE command = ?', [command]);
