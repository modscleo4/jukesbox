 ...
 *
 * @param {string} database_url
 * @return {Promise<{command: string, used: number}[]>}
 */
export async function loadUsageStats(database_url) {
    const db = new DB(database_url);
 ...
 * @param {boolean} [params.deleteOnEnd=false]
 * @return {Promise<*>}
 */
export async function pageEmbed({ client }, { title, description = ' ', content, deleteOnEnd = false }) {
    ...
    function buildEmbed(page) {
        return new MessageEmbed({
            ...
            footer: pages === 0 ? null : { text: `Página ${page} de ${pages}` },
        });
    }
